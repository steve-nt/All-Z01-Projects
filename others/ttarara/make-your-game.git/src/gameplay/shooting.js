...
export function createShootingSystem({
    ...
    function attemptShot(clientX, clientY) {
        ...
        for (let i = entities.length - 1; i >= 0; i--) {
            ...
            if (
                ...
                if (entity.isTurtle || entity.variant === 'turtle') {
                    ...
                    state.setScore(Math.max(0, state.getScore() - turtlePenalty));
                    ...
                    // Show penalty popup
                    showMissPopup(entity.x, entity.y, turtlePenalty);
                ...
                // Regular fish — increment combo and score
                    ...
                    feedback.showScorePopup(entity.x, entity.y, points, combo);
                }
        ...
        if (!hitSomething) {
            ...
            state.setScore(Math.max(0, state.getScore() - missPenalty));
            showMissPopup(x, y, missPenalty);
            state.setCombo(0);
            state.setComboTimer(0);
            feedback.updateComboDisplay();
        }

...
type groupEventResponse struct {
	...
	CreatedAt    time.Time `json:"created_at"`
	// UserResponse is the response of the currently logged-in user for this event (if any).
	// Can be null when the user hasn't responded yet.
...
// Returns pending invitations for the logged-in user.
func ListGroupInvitationsHandler(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		writeJSONError(w, http.StatusMethodNotAllowed, "method_not_allowed", "Method not allowed")
		return
	}
	...
	rows, err := db.Query(`
		SELECT
			...
			g.group_name,
			gi.inviter_id,
			COALESCE(NULLIF(u.nickname, ''), u.email, '') AS inviter_name,
			gi.invitee_id
	...
	`, uid)
	if err != nil {
		writeJSONError(w, http.StatusInternalServerError, "db_error", "failed to fetch invitations")
		return
	}
	defer rows.Close()
	...
	case http.MethodGet:
		...
		rows, err := db.Query(`
			SELECT
				...
				e.event_datetime,
				e.created_at,
				gre.response AS user_response
			FROM Group_Events e
			LEFT JOIN Group_Event_Responses gre
				ON gre.group_event_id = e.id AND gre.user_id = ?
			WHERE e.group_id = ?
		...
		for rows.Next() {
			...
			if userResp.Valid {
				e.UserResponse = &userResp.String
			} else {
				e.UserResponse = nil
			}

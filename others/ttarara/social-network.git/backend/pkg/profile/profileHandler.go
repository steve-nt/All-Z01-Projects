...
func FollowAcceptHandler(w http.ResponseWriter, r *http.Request) {
	...
	// If the profile owner is public, follow requests are auto-accepted.
	var isPublic bool
	err = db.QueryRow(`SELECT is_public FROM Users WHERE user_id = ?`, currentUserID).Scan(&isPublic)
	if err != nil {
		writeJSONError(w, http.StatusInternalServerError, "failed to fetch profile privacy", "internal_error")
		return
	}
	if isPublic {
		writeJSONError(w, http.StatusBadRequest, "follow requests are not available for public profiles", "invalid_request")
		return
	}
...
func UsersSearchHandler(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		...
		// Prefer a "full name" (first + last), then nickname, then email.
		const empty = ""
		fullName := strings.TrimSpace(strings.TrimSpace(first) + " " + strings.TrimSpace(last))
		if fullName != empty {
			user.UserName = fullName
		} else if user.Nickname != empty {

...
export const useGroupsStore = defineStore("groups", {
  ...
  actions: {
    ...
    async respondToEvent(eventId, response, groupId) {
      ...
      this.respondingEventId = normalizedEventId;
      try {
        await socialApi.respondToGroupEvent(normalizedEventId, response);
        const gid =
          groupId ??
          this.groupEvents.find((e) => Number(e.event_id) === normalizedEventId)?.group_id;
        if (gid) await this.fetchGroupEvents(gid);
        return response;
      } catch (error) {
        this.error = {
          status: error?.status || 500,
          message: error?.message || "Failed to respond to event"
        };
        throw error;
      } finally {

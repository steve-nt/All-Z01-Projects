...
router.isReady().then(async () => {
  ...
  wsService.on("message", (data) => {
    ...
    if (data.type === "notification") {
      ...
      const isFollowRequest = payload?.type === "follow_request";
      const isNewFollower = payload?.type === "new_follower";
      const isFollowRelatedToast = typeof payload?.type === "string" && payload.type.includes("follow");
      // Toast: only for non-message types (we already show toast in private_message / group_message handlers)
      // Also suppress follow-related toast popups (follow requests page handles them).
      if (
        !isPrivateMessage &&
        !isGroupMessage &&
        !isFollowRequest &&
        !isNewFollower &&
        !isFollowRelatedToast
      ) {
      ...
      // Keep the group invitations tab in sync in real-time.
      if (payload?.type === "group_invitation") {

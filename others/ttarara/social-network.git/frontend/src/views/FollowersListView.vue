<template>
  <div class="page-card">
    <h1>Followers</h1>
    <p v-if="followStore.loading" class="muted">Loading followers...</p>
    <p v-if="localError" class="error">{{ localError }}</p>

    <ul v-if="!followStore.loading && followers.length" class="user-list">
      <li v-for="user in followers" :key="user.user_id" class="user-list-item">
    ...
    <p v-else-if="!followStore.loading" class="muted">No followers found.</p>
  </div>
</template>
...
onMounted(async () => {
  ...
  try {
    ...
    localError.value = error?.message || "Failed to load followers.";
  }
});

      ...
      <section v-if="isMember" class="invite-section">
        ...
        <ul v-else-if="inviteResults.length > 0" class="invite-list">
          <li v-for="user in inviteResults" :key="user.user_id" class="invite-item">
            <span class="invite-user-name">{{ user.user_name || user.nickname || "User #" + user.user_id }}</span>
            <button
              type="button"
              class="button button-small"
          ...
          >
            ...
            <div class="event-actions">
              <button
                type="button"
                class="button button-small"
                :disabled="groupsStore.respondingEventId === Number(ev.event_id)"
                @click="onEventResponse(ev.event_id, 'going')"
              >
                {{
                  groupsStore.respondingEventId === Number(ev.event_id)
                    ? "..."
                    : resolvedEventResponse(ev) === "going"
                      ? "Going ✓"
                      : "Going"
                }}
              </button>
              ...
              >
                {{
                  groupsStore.respondingEventId === Number(ev.event_id)
                    ? "..."
                    : resolvedEventResponse(ev) === "not going"
                      ? "Not going ✓"
                      : "Not going"
                }}
              </button>
...
const eventResponseById = ref({});

function resolvedEventResponse(ev) {
  const id = Number(ev?.event_id);
  const local = eventResponseById.value?.[id];
  // Use optimistic local value first; fall back to backend value after refresh.
  return local || ev?.user_response || null;
}
...
async function onEventResponse(eventId, response) {
  ...
  try {
    await groupsStore.respondToEvent(normalizedEventId, response, groupId.value);
    eventResponseById.value = {
      ...eventResponseById.value,
      [normalizedEventId]: response
    };
  } catch (err) {
    // store may have set the error; ensure the banner has something useful
    if (!groupsStore.error) {
      groupsStore.error = {
        status: err?.status || 500,
        message: err?.message || "Failed to update event response"
      };
    }
    // Useful for verifying the click triggers an API request
    console.error("[GroupView] onEventResponse failed", { eventId: normalizedEventId, response, err });
  }
}

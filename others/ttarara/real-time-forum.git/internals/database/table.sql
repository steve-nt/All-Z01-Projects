-- Users Table: stores registered users and credentials (passwords are hashed)
CREATE TABLE IF NOT EXISTS Users (
    user_id           INTEGER PRIMARY KEY AUTOINCREMENT,
    username          TEXT    NOT NULL UNIQUE, -- nickname
...
-- Images Table: stores uploaded image metadata; created before Posts to satisfy FK constraints
...
-- Posts Table: stores user posts and optional cover image reference
...
-- Comments Table: stores user comments linked to posts
...
-- Notifications Table: stores user notifications; related_* FKs allow linking to content and users
...
-- PrivateMessages Table: stores private messages between users
...
-- PrivateMessages indexes
CREATE INDEX IF NOT EXISTS idx_private_messages_sender ON PrivateMessages(sender_id, creation_date DESC);
CREATE INDEX IF NOT EXISTS idx_private_messages_receiver ON PrivateMessages(receiver_id, creation_date DESC);
CREATE INDEX IF NOT EXISTS idx_private_messages_conversation ON PrivateMessages(sender_id, receiver_id, creation_date DESC);
CREATE INDEX IF NOT EXISTS idx_private_messages_reverse_conversation ON PrivateMessages(receiver_id, sender_id, creation_date DESC);

// Package middleware: cross-cutting HTTP middleware for logging and panic recovery.
/*
Responsibilities:
- Wrap handlers with request logging and panic recovery.
- Capture HTTP status codes for audit logs.
- Ensure panics surface as a consistent 500 response via handlers.InternalServerErrorHandler.
*/
package middleware
...
func LoggingMiddleware(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Create a custom ResponseWriter to capture status code without altering handler behavior.

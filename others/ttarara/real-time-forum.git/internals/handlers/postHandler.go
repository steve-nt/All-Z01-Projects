...
func CreatePostHandler(w http.ResponseWriter, r *http.Request) {
	...
	// 🔔 REAL-TIME: ενημέρωση όλων των συνδεδεμένων clients μέσω WebSocket
	// Δεν στέλνουμε όλο το post (για να μην διπλασιάζουμε λογική),
	// απλά ένα event "new_post" ώστε τα clients να καλέσουν ξανά loadPosts().
	event := map[string]string{
		"type": "new_post",
	}
	if jsonEvent, err := json.Marshal(event); err == nil {
		// wsBroadcast είναι στο wsHandler.go, ίδιο package (handlers) άρα προσβάσιμο.
...
func PostsAPIHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		json.NewEncoder(w).Encode([]database.PostResponse{})
		return
	}
	defer rows.Close()

	// Initialize as empty slice to ensure it's always an array
	posts := make([]database.PostResponse, 0)
	for rows.Next() {

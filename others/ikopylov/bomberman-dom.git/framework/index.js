...
/**
 * Apply props to a DOM element. If context is provided, props.on is stored in context.handlerMap
 * (delegation) instead of adding per-element listeners, so rerenders do not double-fire.
 * @param {Element} el
 * @param {Object} props
 * @param {RenderContext} [context]
 */
function applyProps(el, props, context) {
  const p = props ?? {};
  if (context) {
...
function patch(el, oldVNode, newVNode, context) {
  ...
  applyProps(domEl, newVNode.props, context);

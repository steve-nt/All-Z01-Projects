...
void systemWindow(const char *id, ImVec2 size, ImVec2 position)
{
    ...
    ImGui::Text("User Name: %s", getUsername().c_str());
    ImGui::Text("Hostname: %s", getHostname().c_str());
    ImGui::Text("Number of Processes: %d", countProcesses());
    ImGui::Text("CPU Type: %d", CPUinfo().c_str());
    ...
    if (ImGui::BeginTabBar("PerformanceTabs"))
    {
        ...
        {
            auto speeds = getFanSpeeds();
            if (speeds.empty())
            {
                ImGui::Text("No fans detected or insufficient permissions.");
            }
            else
            {
                for (size_t i = 0; i < speeds.size(); ++i)
                {
                    ImGui::Text("Fan %zu: %d RPM", i, speeds[i]);
                }
            }
            ImGui::EndTabItem();
        }
        ...
        {
            auto zones = getThermalZones();
            if (zones.empty())
            {
                ImGui::Text("No thermal zones detected.");
            }
            else
            {
                for (auto &z : zones)
                {
                    ImGui::Text("%s: %.1f °C", z.first.c_str(), z.second);
                }
            }
            ImGui::EndTabItem();
        }
...
void memoryProcessesWindow(const char *id, ImVec2 size, ImVec2 position)
{
    ...
    ImGui::Text("Memory and Processes Window");
    ImGui::Text("TODO: Show RAM usage, top processes...");
...
void networkWindow(const char *id, ImVec2 size, ImVec2 position)
{
    ...
    ImGui::Text("TODO: Show RX/TX per interface...");
...
int main(int, char **)
{
    ...
    ImGui_ImplSDL2_InitForOpenGL(window, gl_context); // Connect SDL2 to ImGui
    ImGui_ImplOpenGL3_Init(glsl_version);             // Connect OpenGL to ImGui
    ...
    while (!done)
    {
        ...
        SDL_Event event;
        while (SDL_PollEvent(&event)) // Check for quit or window close
        {
        ...
        ImGui::NewFrame(); // Start a new ImGui GUI frame
        ...
        // Rendering, This actually draws the GUI on the screen.
        ImGui::Render();
    ...
    // Cleanup, Frees memory and shuts down SDL.

...
func GetTerminalSizeWin() (int, int, error) {
	width := 0
	cmd := exec.Command("mode", "con") // This command retrieves the current size of the terminal.
	cmd.Stdin = os.Stdin               // Set the command's standard input to the terminal's standard input.
	out, err := cmd.Output()           // Execute the command and capture the output.
	for _, data := range strings.Split(string(out), "\n") {
		if strings.HasPrefix(strings.TrimSpace(data), "Col") {
			width, err = strconv.Atoi(strings.TrimSpace(strings.Split(strings.TrimSpace(data), ":")[1]))
		}
	}
	return width, 0, err
}

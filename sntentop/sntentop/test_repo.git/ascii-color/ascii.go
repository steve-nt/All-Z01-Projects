...
func PrintMultipleCharacter(s string, asciiTemplates [][]string) {
	tempIntArrLetter := ReturnAsciiCodeInt(s, asciiTemplates) // Pass asciiTemplates here
	for i := 0; i < 8; i++ {
...
// ReturnAsciiCodeInt converts each character to its ASCII template index.
func ReturnAsciiCodeInt(s string, asciiTemplates [][]string) []int {
	var tempIntArrLetter []int
	for _, v := range s {
		index := int(v) - 32
		// Ensure index is within bounds of the ASCII template range
		if index < 0 || index >= len(asciiTemplates) {
			fmt.Printf("Warning: Character '%c' is out of range for ASCII art\n", v)
			continue // Skip characters that don't have an ASCII art template
		}
		tempIntArrLetter = append(tempIntArrLetter, index)
	}

...
func main() {
	...
	fmt.Println("Server running at: http://localhost:2020/")
	startTime := time.Now()
	...
	mux.HandleFunc("/geolocations", backend.GeocodeAddressHandler) // Use GeocodeAddressHandler here
	// Start the HTTP server on port 2020 using the custom ServeMux

#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════════
# PODMAN MANAGEMENT HELPER SCRIPT FOR RUNNING forum-image-upload
...
show_usage() {
    cat << EOF
════════════════════════════════════════════════════════════
Running forum-image-upload (Podman Helper)
════════════════════════════════════════════════════════════
...
cmd_backup() {
    ...
    podman cp running-forum-image-upload:/home/appuser/app/data/forum-image-upload.db "$BACKUP_FILE"
...
cmd_restore() {
    ...
    if [ "$confirm" = "YES" ]; then
        podman cp "$LATEST" running-forum-image-upload:/home/appuser/app/data/forum-image-upload.db
        podman-compose restart
...
cmd_shell() {
    ...
    podman exec -it running-forum-image-upload sh
}

cmd_health() {
    print_info "Health status:"
    podman inspect --format '{{.State.Health.Status}}' running-forum-image-upload || echo "Not running"
}

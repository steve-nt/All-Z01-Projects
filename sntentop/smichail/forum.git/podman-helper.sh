#!/bin/bash
# ═══════════════════════════════════════════════════════════════════════════
# PODMAN MANAGEMENT HELPER SCRIPT FOR RUNNING FORUM
...
cmd_backup() {
    ...
    podman cp running-forum:/home/appuser/app/data/forum.db "$BACKUP_FILE"
...
cmd_restore() {
    ...
    if [ "$confirm" = "YES" ]; then
        podman cp "$LATEST" running-forum:/home/appuser/app/data/forum.db
        podman-compose restart
...
cmd_shell() {
    ...
    podman exec -it running-forum sh
}

cmd_health() {
    print_info "Health status:"
    podman inspect --format '{{.State.Health.Status}}' running-forum || echo "Not running"
}

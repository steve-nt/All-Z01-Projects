...
def company_dashboard(request):
    """Dashboard for companies to view their service requests"""
    # Check if user is authenticated
    if not request.user.is_authenticated:
        messages.info(request, 'Please log in to access the company dashboard.')
        return redirect('login_user')
    
    # Check if user is a company
    if not request.user.is_company:
        messages.error(request, 'Only companies can access this page.')
        logger.warning(f'Non-company user {request.user.username} attempted to access company dashboard')
        return redirect('services_list')
    ...
    # Separate requests by status for easy filtering
    pending_requests = service_requests.filter(status='pending')
    accepted_requests = service_requests.filter(status='accepted')
    rejected_requests = service_requests.filter(status='rejected')
    ...
    return render(request, 'services/company_dashboard.html', context)
